...
> ### ⚠️ Experimental build for SPT 4.1.x
>
> This is v1.0.0 ported to the SPT 4.1 APIs. It builds, loads and works in a raid, but it has had far
> less in-game testing than the stable build. For the tested version (SPT 4.0.13), switch to the
> [`main`](../../tree/main) branch.

**Requires SPT 4.1.0 or newer. Built and tested on 4.1.2.**
...
* Support for the [Batteries Not Included](https://forge.sp-tarkov.com/mod/2513/batteries-not-included) mod — **inactive on this branch**, since that mod has no 4.1 release yet. The radios simply run as if no battery mod were installed; this is the intended fallback, not a bug.

## Installation

1. Open the downloaded archive with 7zip (recommended)
2. Locate the mod folder inside the 7zip archive
3. Drag the selected folders (`BepInEx` and `SPT_Runtime`) into your SPT client folder

> **Note:** the folder layout changed in SPT 4.1 — the server side now goes to `SPT_Runtime\user\mods\`, **not** `SPT\user\mods\`.
...
* **This is an experimental 4.1 port** — less tested than the stable build;
