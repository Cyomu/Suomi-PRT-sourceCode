...
{
    [BepInPlugin("com.suomi.radiomod", "Suomi-PRT", "0.9.6")]
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            _duplexModeModifier = Config.Bind(
                ...
                "Duplex / Simplex Mode",
                KeyCode.Return,
                Desc("Toggle Half-duplex/Simplex (whether you can hear others while transmitting; not available on every radio)", 10));
            ...
            _showDuplexIndicator = Config.Bind(
                "4. Indicators",
                "Duplex/Simplex Indicator",
                true,
                Desc("Small dot showing the current Half-duplex/Simplex mode. Only visible while the radio is on.", 40));
            ...
            _colorSimplex = Config.Bind(
                ...
                "Simplex Mode Color",
                Color.yellow,
                Desc("Color used for the Duplex/Simplex indicator (Simplex) and the matching notification text", 30));
        ...
        private static bool IsEnglishLanguage()
        {
            try
            {
                return LocaleManagerClass.LocaleManagerClass?.String_1 == LocaleManagerClass.ENGLISH_LOCALIZATION;
            }
            catch
            {
                return false;
        ...
        private void EnforceRadioLocation(DissonanceComms comms)
        {
            ...
            if (_radioOn && _radioLocation == RadioLocation.None)
            {
                ...
                Notify(L("Рация выключена (нет в снаряжении)", "Radio turned off (not in gear)"), null);
            }
        ...
        private RadioLocation GetRadioLocation()
        {
            ...
            if (_selectedRadioTplId == null || !existsAnywhere.Contains(_selectedRadioTplId))
            {
                ...
                {
                    Notify(L("Выбрана рация: ", "Radio selected: ") + GetRadioDisplayName(_selectedRadioTplId)
                        + L(" (R.Shift+VOIP — сменить)", " (R.Shift+VOIP to switch)"), _colorSelect.Value);
                }
            }
        ...
        private void SelectNextRadio()
        {
            ...
            if (local == null || local.Inventory == null || local.Inventory.Equipment == null)
            {
                Notify(L("Нет доступа к инвентарю", "No access to inventory"), null);
                return;
            }
            List<string> available = CollectSelectableRadioTpls(local.Inventory.Equipment);
            if (available.Count == 0)
            {
                Notify(L("Нет рации в разгрузке/кармане для выбора", "No radio in vest/pocket to select"), null);
                return;
            }
            if (available.Count == 1)
            {
                _selectedRadioTplId = available[0];
                Notify(L("Только одна рация доступна: ", "Only one radio available: ") + GetRadioDisplayName(_selectedRadioTplId), _colorSelect.Value);
                _radioLocation = GetRadioLocation();
                return;
            }
        ...
        private void CheckLocalVoice(DissonanceComms comms)
        {
            ...
            if (risingEdge)
            {
                ...
                if (duplexModifier)
                {
                    if (!_radioOn)
                    {
                        Notify(L("Сначала включите рацию", "Turn on the radio first"), null);
                        return;
                    }
                    if (_activeRadioTplId == null || !SimplexCapableTplIds.Contains(_activeRadioTplId))
                    {
                        Notify(L("Переключение режима недоступно.", "Mode switching unavailable."), null);
                        return;
                    }
                    if (_radioLocation == RadioLocation.Backpack)
                    {
                        Notify(L("Рация в рюкзаке — нет доступа", "Radio is in backpack — no access"), null);
                        return;
                    }
                    _duplexMode = _duplexMode == DuplexMode.HalfDuplex ? DuplexMode.Simplex : DuplexMode.HalfDuplex;
                    Notify(_duplexMode == DuplexMode.Simplex ? L("Режим: Симплекс", "Mode: Simplex") : L("Режим: Полудуплекс", "Mode: Half-duplex"),
                ...
                if (toggleModifier)
                {
                    if (!_radioOn && _radioLocation == RadioLocation.None)
                    {
                        ...
                        Notify(L("Нет рации в снаряжении", "No radio in gear"), null);
                        return;
                    }
                    if (_radioLocation == RadioLocation.Backpack)
                    {
                    ...
                    _radioOn = !_radioOn;
                    ApplyReceiveState(comms);
                    Notify(_radioOn ? L("Рация включена", "Radio on") : L("Рация выключена", "Radio off"), _radioOn ? _colorOn.Value : (Color?)null);
